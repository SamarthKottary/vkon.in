 ...
 * be changed until noon the next day and the admin can book from noon the next
 * day; written in two places they would drift, and a booking made while the
 * customer can still move the parcel is a label with the wrong address on it.
 *
...
/** 12:00 noon, Indian time, on the calendar day after `iso`. */
export function nextDayNoonIST(iso: string): Date {
  /* Shifted so the UTC fields read as Indian wall-clock time — then "the next
     day at 12:00" is plain date arithmetic, and shifted back. */
  const local = new Date(new Date(iso).getTime() + IST_OFFSET_MS);
  return new Date(
    Date.UTC(local.getUTCFullYear(), local.getUTCMonth(), local.getUTCDate() + 1, 12, 0, 0) -
      IST_OFFSET_MS,
  );
}
 ...
 * next-day-noon clock. Cash on delivery counts from placing it; an online
...
export function addressDeadline(order: WindowFields): Date | null {
  ...
  return from ? nextDayNoonIST(from) : null;
}
...
/** "12 pm on Sat, 19 Sep", in Indian time wherever the server is. */
export function formatNoonDeadline(iso: string): string {
