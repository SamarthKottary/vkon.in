...
### 2026-09-01 (home, featured products) — Mobile `snap-mandatory` reverted; it reintroduced the desktop vertical-scroll glitch this file already root-caused, now on touch

**Client: "i feel some glitch when feature card scrolling vertically."** Earlier the same day, `correctCardStep` (the "both cards half-visible" fix, entries above) shipped paired with CSS `snap-x snap-mandatory` below `sm`, reasoned at the time as safe because "a touch swipe carries none of that wheel-noise risk" `mandatory` was already known to cause on desktop. That reasoning covered a deliberate horizontal swipe but not this report's gesture: an incidental *vertical* page-scroll that happens to start with a finger over a card. `mandatory` snap claims any gesture that ends near a snap point regardless of why it happened — the exact mechanism this file spent all of 2026-08-24 root-causing for desktop wheel/trackpad input (see that day's several entries) and fixed there by switching to `proximity` — and a mostly-vertical touch scroll carrying a few incidental pixels of horizontal drift over this row was getting grabbed the same way.

**Reverted to plain `snap-proximity` at every width; `correctCardStep` is untouched and does not need `mandatory` to work.** It is a JS settle-timer correction (already shipped, entries above) that forces an exact card-boundary rest position on top of whatever CSS snap is doing — it still does that job under `proximity`, just without the browser's own strict snap grabbing gestures it was never meant to touch. Solves "both cards visible" without reopening the vertical-scroll bug a second time.

Verified by re-reading the 2026-08-24 root-cause entries this regression exactly reproduces the shape of, rather than on a real phone — no browser-automation tool was available in this session; worth confirming vertical page-scroll now chains cleanly over the row on real touch hardware, alongside the other unverified mobile items in this log. `npx tsc --noEmit` not run in this session (no local Node toolchain available); change is a CSS class removal with no new types.
...
Fixed by calling `updatePopProgress(el)` directly from the mount effect (right after `sync()`) and from its `ResizeObserver` callback, rather than leaving it to only ever run inside the drag/autoplay loop. `updatePopProgress` already re-finds `centeredId` on every call the same way `measure()` does, so this costs one extra per-card scan on mount and on resize, nothing ongoing — the loop itself, and everything gating when it runs, is untouched.

**Follow-up, same day (client: "in desktop view quick view only visible when mouse pointed to it only, some thing from the mobile view effected desktop view"): the seed call above was missing the `!hoverCapable` gate `runPopProgressLoop` itself always carries.** `--pop-progress` is a touch-only mechanism everywhere else in this file; a pointer device's Quick View is meant to run on `group-hover` alone, with the property left unset so `var(--pop-progress,0)` falls back to its CSS default. Calling `updatePopProgress` unconditionally wrote a real, non-zero value onto a pointer device's cards too — whichever one sat nearest the track's centre at that instant — fighting `group-hover` intermittently instead of leaving the desktop path untouched. Both the mount call and the `ResizeObserver` callback's call are now gated on `!hoverCapable`; the observer itself still attaches and still calls `sync()` unconditionally, since that measurement (`canScroll`, the dots, `centeredId`) was never touch-only and was not part of the regression.

Verified: reasoned through the timeline (mount now writes `--pop-progress` for the initially-centred card before first paint of its own value, independent of any interaction, and only on a device without `:hover`) rather than confirmed on a real phone or desktop browser — no browser-automation tool was available in this session; worth confirming both the original mobile fix and this desktop correction on real hardware alongside the other unverified items in this log. `npx tsc --noEmit` not run in this session (no local Node toolchain available); change reuses an existing, already-typed function and adds no new types.

### 2026-09-01 (home, featured products) — Quick View moves from dead-centre to 25% up from the image's bottom edge

**Client: "place quick view in the feature card 25% above from the bottom of the image," then, same day, three follow-ups: "make 15% from bottom," "i changed to 10%," then "change quick view to 25% again."** `FeaturedCard`'s Quick view wrapper (both branches — with and without a photo) was `top-1/2 -translate-y-1/2`, vertically centred on the image. Now `bottom-[25%]` with no vertical translate — back to the value first asked for, after two intermediate values (`15%`, `10%`) along the way — so its own centre sits 25% of the image's height up from the bottom edge. Horizontal centring (`left-1/2 -translate-x-1/2`) is unchanged, as is everything driving its opacity (`--pop-progress` on touch, `group-hover` on a pointer) and the `data-featured-quickview` autoplay-pause marker.

Verified: Quick view now renders 25% up from the image's bottom edge on both the with-photo and no-photo `FeaturedCard` branches, matching each other. `npx tsc --noEmit` not run in this session (no local Node toolchain available); change is a plain Tailwind class swap with no new types.
