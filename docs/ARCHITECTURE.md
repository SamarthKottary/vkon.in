...
**Follow-up, same day (client: "keep fade effect between center to +-20% fades 0 to 100%", then "changes to +-30%"): `verticalCloseness` is now a plain linear ramp, not the horizontal factor's plateau-then-ramp shape.** 100% right at the viewport's vertical centre, falling straight to 0% at ±30% of the viewport's own height away from it — the initial pass had copied the horizontal factor's flat plateau (unfaded for an inner band, only ramping outside it), which is not what was asked for here. The horizontal factor is untouched; only the vertical one changed shape.
